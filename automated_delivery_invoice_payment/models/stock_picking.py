...
class StockPicking(models.Model):
    ...
    created_invoice_id = fields.Many2one('account.move', readonly=1, 
        help="This sale invoice was automatically created after this "
        "transfer was validated. Field used to print the invoice at picking. "
        "Invoice was created by pressing the create invoice from sale order.")
    ...
    def button_validate(self):
        ...
        if (res == True and 
            not self.env.context.get('skip_create_invoice_after_transfer')):
            ...
            for rec in self.sudo(): 
                if rec.sale_id and rec.sale_id.invoice_status == "to invoice":  
                    ...
                    sale_to_write = {
                        'paid_less':paid_less, 
                        'paid_more':paid_more, 
                        'difference_between_order_and_deliverd':False
                    }
                    ...
                    try:
                        ...
                        _logger.error(
                            f'we could not create the invoice from picking={rec} validate because of error:{ex}.\n'
                            f' Normally another module has some requirements and can not be done this and also that'
                            f' ( rma for example with a replace and refund )')
                        created_invoice = ''
                        continue
                    if created_invoice and type(created_invoice) is not dict:  
                        # can be the case when it returns a page with error an not an error
                        if rec.sale_id.authorized_transaction_ids:
                            authorized_tranzactions = rec.sale_id.authorized_transaction_ids.filtered(
                                                        lambda r:r.state=='authorized')
                            ...
                            if  paid_less or  paid_more:
                                ...
                                if paid_less == paid_more:
                                    invoice_to_write['resolved_difference'] = (
                                        f'Modified payment on delivery from {paid} to {created_invoice.amount_total}'
                                    ) 
                        ...
                        try:
                            ...
                            _logger.error(
                                f'we could not post the invoice from picking={rec}, invoice {created_invoice}'
                                f' because of error:{ex}.\n Normally another module has some requirements and'
                                f' can not be done this and also that ')
                            continue
                        ...
                        if rec.sale_id.authorized_transaction_ids:
                            #20211027   modified with try because the the payment_action_capure at sale order 
                            #for bt pay is giving error Payment must be in approved state
                            try:
    ...
    def print_created_invoice(self):
        ...
        if self.created_invoice_id:
            ...
            return self.env.ref('account.account_invoices').report_action(docids, config=False)              
            # config should be false as otherwise it will call configuration wizard that works weirdly
        else:

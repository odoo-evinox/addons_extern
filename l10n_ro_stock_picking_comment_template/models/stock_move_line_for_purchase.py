...
class StockMoveLine(models.Model):
    ...
    purchase_line = fields.Many2one(
        related="move_id.purchase_line_id", string="Related purchase line"
    )
    purchase_currency_id = fields.Many2one(
        related="purchase_line.currency_id", string="Purchase Currency"
    )
    purchase_tax_id = fields.Many2many(
        related="purchase_line.taxes_id", string="Purchase Tax"
    )
    ...
    purchase_tax_description = fields.Char(
        ...
        string="Purchase Tax Description",
        compute_sudo=True,
    )
    purchase_price_subtotal = fields.Monetary(
        compute="_compute_purchase_order_line_fields",
        string="Purchase Price subtotal",
        compute_sudo=True,

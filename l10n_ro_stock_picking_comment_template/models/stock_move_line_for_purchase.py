...
class StockMoveLine(models.Model):
    ...
    purchase_price_subtotal = fields.Monetary(
        compute="_compute_purchase_order_line_fields",
        string="Purchase Price subtotal", compute_sudo=True, currency_field='purchase_currency_id')
            ...
            for move_line in self:
                ...
                if line_key not in aggregated_move_lines:
                    aggregated_move_lines[line_key] = {'name': name,
                                                       ...
                                                       'subtotal_internal_consumption': move_line.subtotal_internal_consumption,
                                                       }
                else:
                    aggregated_move_lines[line_key]['qty_done'] += move_line.qty_done
                    # added by us
    ...
    def _get_unit_price_internal_consumption(self):
        if self.move_id._is_internal_transfer():
            ...
            if pl:
                partner_price = pl.price_get(self.move_id.product_id.id, self.move_id.product_qty)[pl.id]

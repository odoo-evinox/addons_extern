...
# just tracking
    ...
    def create(self,values):
        ...
        values2=values.copy()
        values2.update({'payment_type':'outbound' if values.get('inbound') else 'inbound',
                        'journal_id':values.get('transfer_journal_id'),
                        'transfer_journal_id':values.get('journal_id'),
                        'memo':"REVERS OF " + values.get('memo'),
                        'name':'/',
    ...
    def write(self,values):
        ...
        if not is_internal_transfer:
            ...
            if internal_transfers:
                keys = values.keys()
                if 'date' in keys or 'pyment_type' in keys or 'amount' in keys or 'journal_id' in keys or 'transfer_journal_id' in keys:
        ...
        for rec in self:
            if rec.is_internal_transfer and rec.transfer_related_payment_id:
                if rec.transfer_related_payment_id not in self:
                    rec.transfer_related_payment_id.unlink()
        return super().unlink()

    def action_post(self):
        ''' draft -> posted '''
        self.move_id._post(soft=False)  # till here was the original behavior
        transfer_related_payment_id = self.filtered(lambda r:r.transfer_related_payment_id != False)

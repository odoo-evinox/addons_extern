...
class TestResPartner(TransactionCase):
    ...
    def test_general_template(self):
        ...
        self.assertEqual(templ, self.before_template_id)
        templ = self.partner_id.get_comment_template("after_lines")
        self.assertEqual(templ, self.after_template_id)
    ...
    def test_partner_template(self):
        ...
        templ = self.partner_id.get_comment_template(
            "before_lines", self.partner2_id.id
        )
        self.assertEqual(templ, self.before_template_id)
        templ = self.partner_id.get_comment_template("before_lines", self.partner_id.id)
        self.assertFalse(templ)
        templ = self.partner_id.get_comment_template("after_lines")
        self.assertEqual(templ, self.after_template_id)
